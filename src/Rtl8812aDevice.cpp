...
bool Rtl8812aDevice::send_packet(const uint8_t *packet, size_t length) {
  ...
  _logger->info(
      "radiotap length is {},80211 length is {},usb_frame length should be {}",
  ...
  SET_TX_DESC_RATE_ID_8812(
      usb_frame, static_cast<uint8_t>(rate_id)); // 原来设置的是7，得考虑下
  ...
  SET_TX_DESC_TX_RATE_8812(
      usb_frame, static_cast<uint8_t>(MRateToHwRate(
                     fixed_rate))); // 原来设置的是6,也需要考虑下怎么转换
...
SelectedChannel Rtl8812aDevice::GetSelectedChannel() { return _channel; }
