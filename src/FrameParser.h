...
struct tx_desc {
    unsigned int txdw0; // 第一个描述符字段，通常包含数据包的长度和其他控制信息
    unsigned int txdw1; // 第二个描述符字段，可能包含 MAC ID、队列选择等
    unsigned int txdw2; // 第三个描述符字段，保留或特定用途
    unsigned int txdw3; // 第四个描述符字段，序列号和帧控制标志
    unsigned int txdw4; // 第五个描述符字段，更多控制标志，比如发送速率、状态等
    unsigned int txdw5; // 第六个描述符字段，保留
    unsigned int txdw6; // 第七个描述符字段，可能用于额外的状态信息
    unsigned int txdw7; // 第八个描述符字段，标识包的处理方式

    unsigned int txdw8; // 在40字节描述符中包含的附加字段
    unsigned int txdw9; // 另一个附加字段
};
